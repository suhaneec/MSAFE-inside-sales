...
def hdr(col, label, cls='col-hdr', tip=''):
    ...
    col.markdown(f"<div class='{cls}'{title}>{label}</div>", unsafe_allow_html=True)
...
st.markdown(
    ...
    f"{TODAY_NOW.strftime('%d %b %Y')}</span></div>", unsafe_allow_html=True)
...
st.markdown("<p class='note'>"
            "RAG: 🔴 Win% &lt;2%  🟡 2–5%  🟢 &gt;5% — sorted red to green. "
...
if tc2.button(f"{'▼' if st.session_state.exp_act else '▶'} Active ({len(act_stages)} stages)",
              key='tog_act'):
    st.session_state.exp_act = not st.session_state.exp_act; st.rerun()
if tc3.button(f"{'▼' if st.session_state.exp_lost else '▶'} Lost ({len(lost_stages)} stages)",
              key='tog_lost'):
...
for rep,rd,n,pct7,rs in act4_rows:
    ...
    for i,b in enumerate(BKTS3):
        bd=rd[rd['bkt']==b]
        nbtn(c[3+i],len(bd),f"t4b_{rep}_{i}",bd,f"{b} (active, last followup) — {rep}")
...
st.markdown("<p class='note'>"
            "For each rep: total received, won, lost, active from each source. "
            "RAG on overall Win%. Drill into any cell using the selector below the table.</p>",
            unsafe_allow_html=True)
...
for src in src_avail:
    ...
    gt[f'{src} Total']=len(sd); gt[f'{src} Won']=int((sd['Stage']=='Won').sum())
    gt[f'{src} Lost']=int((sd['Stage']=='Lost').sum()); gt[f'{src} Active']=int((sd['Stage']=='Open').sum())
gt['Total']=total; gt['Won']=won; gt['Lost']=lost; gt['Active']=opn; gt['Win %']=f"{wr}%"
rows5.append(gt)
...
def sty_t5(df):
    ...
    for sc in ['Won']:
        if sc in df.columns:
            s[sc]=df[sc].apply(lambda v:'background:#E9F7EF;color:#0A6640;font-weight:700'
                                if isinstance(v,(int,float)) and v>0 else '')
    if 'Win %' in df.columns:
        s['Win %']=df['Win %'].apply(lambda v:(
            'background:#E9F7EF;color:#0A6640;font-weight:700' if isinstance(v,str) and float(v.replace('%',''))>=5
            else('background:#FFFBEB;color:#92400E;font-weight:600' if isinstance(v,str) and float(v.replace('%',''))>=2
                 else 'background:#FEF2F2;color:#B91C1C;font-weight:700')) if isinstance(v,str) and '%' in v else '')
...
st.markdown(
    ...
    f"{total:,} leads in current filter  |  {TODAY_NOW.strftime('%d %b %Y')}</p>",
    unsafe_allow_html=True)
