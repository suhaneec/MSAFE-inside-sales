import streamlit as st
import pandas as pd
import numpy as np
from io import BytesIO
from datetime import date

st.set_page_config(page_title="MSafe Inside Sales", page_icon="📊",
                   layout="wide", initial_sidebar_state="expanded")

st.markdown("""
<style>
:root { --background-color:#F8FAFC; --text-color:#0F172A; --primary-color:#0F2044; }
html,body,[data-testid="stAppViewContainer"],[data-testid="stMain"],
.stApp,.block-container,[data-testid="stMain"]>div {
    background-color:#F8FAFC !important; color:#0F172A !important; }
[data-testid="stMain"] *,.block-container *,
.stMarkdown *,[data-testid="stMarkdownContainer"] * { color:#0F172A !important; }

/* Tabs — dark bar, white pill for selected */
.stTabs [data-baseweb="tab-list"]  { background:#0F2044 !important; border-radius:8px; padding:4px; gap:2px; }
.stTabs [data-baseweb="tab"]       { color:#CBD5E1 !important; font-weight:600; font-size:13px;
                                      border-radius:6px !important; padding:8px 14px !important; }
.stTabs [data-baseweb="tab"]:hover { color:#FFFFFF !important; background:rgba(255,255,255,0.1) !important; }
.stTabs [aria-selected="true"]     { color:#0F2044 !important; background:#FFFFFF !important;
                                      font-weight:700 !important; border-bottom:none !important; }
.stTabs [data-baseweb="tab"] *     { color:inherit !important; }

/* Download buttons */
[data-testid="stDownloadButton"] button {
    background:#0F2044 !important; color:white !important;
    border:none !important; border-radius:6px !important; font-weight:600 !important; }

/* Sidebar */
section[data-testid="stSidebar"],
section[data-testid="stSidebar"]>div,
section[data-testid="stSidebar"]>div:first-child { background:#0F2044 !important; }
section[data-testid="stSidebar"] *   { color:white !important; }
section[data-testid="stSidebar"] hr  { border-color:#2D5F9E !important; }
section[data-testid="stSidebar"] .stMultiSelect div[data-baseweb="select"]>div {
    background:#162955 !important; border:1px solid #2D5F9E !important; border-radius:6px !important; }
section[data-testid="stSidebar"] .stMultiSelect input { color:white !important; caret-color:white !important; }
section[data-testid="stSidebar"] .stMultiSelect [aria-placeholder] { color:#8BAFD4 !important; }
section[data-testid="stSidebar"] [data-baseweb="tag"] { background:#2D5F9E !important; border-radius:4px !important; }
section[data-testid="stSidebar"] [data-baseweb="tag"] span,
section[data-testid="stSidebar"] [data-baseweb="tag"] svg { color:white !important; fill:white !important; }
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] {
    background:#162955 !important; border:2px dashed #2D5F9E !important; border-radius:8px !important; }
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzone"] *,
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzoneInstructions"] span,
section[data-testid="stSidebar"] [data-testid="stFileUploaderDropzoneInstructions"] small,
section[data-testid="stSidebar"] [data-testid="stFileUploader"] span { color:#8BAFD4 !important; }
section[data-testid="stSidebar"] .stDateInput div[data-baseweb="input"],
section[data-testid="stSidebar"] .stDateInput div[data-baseweb="base-input"] {
    background:#162955 !important; border:1px solid #2D5F9E !important; border-radius:6px !important; }
section[data-testid="stSidebar"] .stDateInput input { color:white !important; background:#162955 !important; }

/* KPI cards */
.kpi-block { background:white !important; border-radius:10px; padding:14px 10px;
    text-align:center; box-shadow:0 2px 6px rgba(0,0,0,0.08); height:84px;
    display:flex; flex-direction:column; justify-content:center; border-top:3px solid #CBD5E1; }
.kpi-label { font-size:10px; color:#475569 !important; font-weight:700;
    text-transform:uppercase; letter-spacing:.06em; margin-bottom:5px; }
.kpi-value { font-size:22px; font-weight:700; color:#0F2044 !important; line-height:1; }
.kpi-value.green { color:#0A6640 !important; }
.kpi-value.red   { color:#B91C1C !important; }
.kpi-value.amber { color:#92400E !important; }
.kpi-value.blue  { color:#1D4ED8 !important; }

/* Section headers */
.sec-hdr { background:#0F2044 !important; color:white !important; padding:9px 18px;
    border-radius:6px; font-weight:700; font-size:13px; margin:14px 0 6px 0; }
.sub-note { font-size:12px !important; color:#475569 !important; font-style:italic; margin:0 0 8px 0; }

/* Dataframe */
[data-testid="stDataFrame"] th,[data-testid="stDataFrame"] [role="columnheader"],
[data-testid="stDataFrame"] [role="columnheader"] * {
    background:#1B3A6B !important; color:white !important; font-size:12px !important; font-weight:700 !important; }
[data-testid="stDataFrame"] td,[data-testid="stDataFrame"] [role="gridcell"],
[data-testid="stDataFrame"] [role="gridcell"] * { color:#0F172A !important; font-size:12px !important; }
[data-testid="stDataFrame"] [role="rowheader"],[data-testid="stDataFrame"] [role="rowheader"] * {
    color:#0F172A !important; font-size:12px !important; font-weight:600 !important; background:#F1F5F9 !important; }
</style>
""", unsafe_allow_html=True)

# ── CONSTANTS ─────────────────────────────────────────────────────────────────
LOST_S = ['Not-Interested','Not Search Our Product','Regret','Other Department Working',
          'Wrong Number','No Response on RNR','Already Purchased','Quoted Order Lost']
WON_S  = ['Quoted Order Won And Executed']
HIGH_S = ['Quoted In Follow Up','Quoted Order In Pipeline','Quote In Progress',
          'Interested Quote Sent','Quoted Not Picking Call','Quoted Project On Hold',
          'Quoted Order Won And Executed','Quoted Order Lost']
RNR_S  = ['Call Back','RNR Call Back']
ADMIN  = ['msafe947362','50988-Surbhi']
SRC_MAP = {
    'Just Dial':'JustDial','Justdial':'JustDial',
    'Paid clasifieds':'IndiaMart','Paid classifieds':'IndiaMart','Indiamart':'IndiaMart',
    'Exisiting Client Refrence':'Ex-Client Ref.','Existing Client Reference':'Ex-Client Ref.',
    'IVR CALL':'IVR Call','IVR call':'IVR Call',
    'Popup Enquiry':'Website','Webiste':'Website','Product Detail Page':'Website',
    'SEO Landing Pages (Generic)':'Google Ads','Google-Ad (Generic)':'Google Ads',
    'Advertisement':'Other','Aajjo':'Other',
}
MAIN_SOURCES = ['JustDial','IndiaMart','IVR Call','Existing Client',
                'Ex-Client Ref.','Facebook','Other']
SRC_SHORT    = {'JustDial':'JD','IndiaMart':'IM','IVR Call':'IVR',
                'Existing Client':'EC','Ex-Client Ref.':'ECR','Facebook':'FB','Other':'Oth'}
TODAY = pd.Timestamp('2026-06-04')

# ── DATA LOAD ─────────────────────────────────────────────────────────────────
@st.cache_data(show_spinner="Loading CRM data…")
def load_data(fb):
    df = pd.read_excel(BytesIO(fb), engine='openpyxl')
    df['Stage']  = df['FollowupStatus'].apply(
        lambda x:'Won' if x in WON_S else ('Lost' if x in LOST_S else 'Active'))
    df['Source'] = df['SourceName'].replace(SRC_MAP)
    df['is_admin'] = df['LastFollowupCreatedByName'].isin(ADMIN)
    df['Rep'] = df['LastFollowupCreatedByName'].str.replace('50988-','',regex=False)
    df.loc[df['is_admin'],'Rep'] = 'Admin'
    df['Source_group'] = df['Source'].apply(lambda x: x if x in MAIN_SOURCES else 'Other')
    if 'CreatedOn' in df.columns:
        df['CreatedOn'] = pd.to_datetime(df['CreatedOn'], errors='coerce')
        df['age_days']  = (TODAY - df['CreatedOn']).dt.days
    return df

def age_bucket(d):
    if pd.isna(d) or d < 0: return '0–7 days'
    if d <= 7:  return '0–7 days'
    if d <= 15: return '8–15 days'
    if d <= 30: return '16–30 days'
    return '30+ days'

def rnr_bucket(d):
    if pd.isna(d) or d < 0: return '0–3 days'
    if d <= 3:  return '0–3 days'
    if d <= 7:  return '4–7 days'
    if d <= 15: return '8–15 days'
    return '15+ days'

# ── STYLE HELPERS ─────────────────────────────────────────────────────────────
BASE = {'color':'#0F172A','font-size':'12px','font-weight':'500'}

def c_winpct(v):
    try:
        p=float(str(v).replace('%',''))
        if p>=5:  return 'background:#E9F7EF;color:#0A6640;font-weight:700'
        if p>=2:  return 'background:#FFFBEB;color:#92400E;font-weight:600'
        if p==0:  return 'background:#FEF2F2;color:#B91C1C;font-weight:700'
        return 'color:#64748B'
    except: return ''

def c_losspct(v):
    try:
        p=float(str(v).replace('%',''))
        if p>=70: return 'background:#FEF2F2;color:#B91C1C;font-weight:700'
        if p>=50: return 'background:#FFFBEB;color:#92400E;font-weight:600'
        return 'background:#E9F7EF;color:#0A6640;font-weight:600'
    except: return ''

# ── SIDEBAR ───────────────────────────────────────────────────────────────────
st.sidebar.markdown("## 📊 MSafe CRM")
st.sidebar.markdown("---")
uploaded = st.sidebar.file_uploader("Upload CRM export (.xls / .xlsx)", type=['xls','xlsx'])

if not uploaded:
    _,mid,_ = st.columns([1,2,1])
    with mid:
        st.markdown("<br><br>", unsafe_allow_html=True)
        st.markdown(
            "<div style='text-align:center;padding:48px 24px;background:#FFFFFF;"
            "border-radius:12px;border:1px solid #E2E8F0;'>"
            "<div style='font-size:56px;'>📂</div>"
            "<div style='font-size:22px;font-weight:800;color:#0F2044 !important;"
            "font-family:Arial,sans-serif;margin:16px 0 10px;'>MSafe Inside Sales Dashboard</div>"
            "<div style='font-size:14px;color:#475569;font-family:Arial,sans-serif;'>"
            "Upload your KIT19 CRM export (.xls / .xlsx) from the sidebar.</div>"
            "</div>", unsafe_allow_html=True)
    st.stop()

df_raw   = load_data(uploaded.read())
reps_df  = df_raw[~df_raw['is_admin']].copy()

st.sidebar.markdown("### Filters")
all_reps = sorted(reps_df['Rep'].dropna().unique().tolist())
sel_reps = st.sidebar.multiselect("Rep", all_reps, default=all_reps)

if 'CreatedOn' in reps_df.columns and reps_df['CreatedOn'].notna().any():
    mn=reps_df['CreatedOn'].min().date(); mx=reps_df['CreatedOn'].max().date()
    dr=st.sidebar.date_input("Date range",[mn,mx],min_value=mn,max_value=mx)
else:
    dr=None

st.sidebar.markdown("---")
st.sidebar.markdown(f"**Leads in file:** {len(df_raw):,}")

# Base filtered dataset (rep + date only — stage filter not applied globally
# so aging/RNR tabs always show full picture)
base = reps_df.copy()
if sel_reps: base = base[base['Rep'].isin(sel_reps)]
if dr and len(dr)==2 and 'CreatedOn' in base.columns:
    base = base[(base['CreatedOn'].dt.date>=dr[0])&(base['CreatedOn'].dt.date<=dr[1])]

# Ordered reps by total won desc
rep_order = (base.groupby('Rep')['Stage']
             .apply(lambda x:(x=='Won').sum())
             .sort_values(ascending=False).index.tolist())

# ── HEADER ────────────────────────────────────────────────────────────────────
st.markdown(
    "<div style='background:#0F2044;padding:16px 24px;border-radius:10px;margin-bottom:14px;'>"
    "<span style='color:white !important;font-size:19px;font-weight:700;'>"
    "MSafe Equipments — Inside Sales Dashboard</span>"
    "<span style='color:#8BAFD4;font-size:12px;margin-left:16px;'>KIT19 CRM</span>"
    "</div>", unsafe_allow_html=True)

# ── KPI CARDS ─────────────────────────────────────────────────────────────────
total  = len(base)
won    = (base['Stage']=='Won').sum()
lost   = (base['Stage']=='Lost').sum()
active = (base['Stage']=='Active').sum()
quotes = base['FollowupStatus'].isin(HIGH_S).sum()
rnr    = base['FollowupStatus'].isin(RNR_S).sum()
wr     = round(won/total*100,1) if total else 0
q2w    = round(won/quotes*100,1) if quotes else 0
ec     = base[base['Source']=='Existing Client']
ec_wr  = round((ec['Stage']=='Won').sum()/len(ec)*100,1) if len(ec) else 0

kc = st.columns(9)
def kpi(col,label,value,cls=''):
    col.markdown(f"<div class='kpi-block'><div class='kpi-label'>{label}</div>"
                 f"<div class='kpi-value {cls}'>{value}</div></div>",unsafe_allow_html=True)
kpi(kc[0],'Total Leads', f'{total:,}')
kpi(kc[1],'Won',         f'{won:,}',    'green')
kpi(kc[2],'Lost',        f'{lost:,}',   'red')
kpi(kc[3],'Active',      f'{active:,}', 'amber')
kpi(kc[4],'Win Rate',    f'{wr}%',      'blue')
kpi(kc[5],'Quotes Sent', f'{quotes:,}', 'amber')
kpi(kc[6],'Quote→Win',   f'{q2w}%',    'green')
kpi(kc[7],'Cold RNR',    f'{rnr:,}',   'red')
kpi(kc[8],'EC Win Rate', f'{ec_wr}%',  'green')
st.markdown("<br>", unsafe_allow_html=True)

# ── TABS ──────────────────────────────────────────────────────────────────────
t1,t2,t3,t4,t5,t6 = st.tabs([
    "📊  Rep × Source",
    "🌐  Source Performance",
    "⏳  Active Lead Aging",
    "❌  Lost Reasons by Rep",
    "🔵  Stage Funnel by Rep",
    "📞  RNR Aging by Rep",
])

# ══════════════════════════════════════════════════════════════════════════════
# TAB 1 — REP × SOURCE  (Leads + Won per source, row & column totals)
# ══════════════════════════════════════════════════════════════════════════════
with t1:
    st.markdown("<div class='sec-hdr'>Rep × Source — Leads and Won per Source</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Each source = 2 columns: Leads received  |  Won from that source. "
                "Row totals on the right. Column totals at bottom. "
                "🟢 Green Won cells = conversions present. Win % colour coded.</p>",
                unsafe_allow_html=True)

    if total == 0:
        st.info("No data.")
    else:
        rows = []
        for rep in rep_order:
            rd = base[base['Rep']==rep]
            row = {'Rep': rep}
            for src in MAIN_SOURCES:
                sd = rd[rd['Source_group']==src]
                sh = SRC_SHORT[src]
                row[f'{sh} Leads'] = len(sd)
                row[f'{sh} Won']   = int((sd['Stage']=='Won').sum())
            row['Total Leads'] = len(rd)
            row['Total Won']   = int((rd['Stage']=='Won').sum())
            row['Total Lost']  = int((rd['Stage']=='Lost').sum())
            row['Total Active']= int((rd['Stage']=='Active').sum())
            row['Win %']       = f"{round(row['Total Won']/len(rd)*100,1)}%" if len(rd) else '0%'
            rows.append(row)

        # TOTAL row
        tot = {'Rep':'TOTAL'}
        for src in MAIN_SOURCES:
            sd = base[base['Source_group']==src]
            sh = SRC_SHORT[src]
            tot[f'{sh} Leads'] = len(sd)
            tot[f'{sh} Won']   = int((sd['Stage']=='Won').sum())
        tot['Total Leads'] = total
        tot['Total Won']   = int(won)
        tot['Total Lost']  = int(lost)
        tot['Total Active']= int(active)
        tot['Win %']       = f'{wr}%'
        rows.append(tot)

        df1 = pd.DataFrame(rows).set_index('Rep')

        # Won columns list
        won_cols    = [f'{SRC_SHORT[s]} Won' for s in MAIN_SOURCES] + ['Total Won']
        leads_cols  = [f'{SRC_SHORT[s]} Leads' for s in MAIN_SOURCES] + ['Total Leads']
        summary_cols= ['Total Lost','Total Active','Win %']

        def sty_t1(df):
            s = pd.DataFrame('', index=df.index, columns=df.columns)
            # Won cells → green if > 0
            for col in won_cols:
                if col in df.columns:
                    s[col] = df[col].apply(
                        lambda v: 'background:#E9F7EF;color:#0A6640;font-weight:700'
                        if isinstance(v,(int,float)) and v>0 else 'color:#94A3B8')
            # Total Leads col light highlight
            if 'Total Leads' in df.columns:
                s['Total Leads'] = 'background:#EAF0FB;color:#0F2044;font-weight:700'
            # Win % colour
            if 'Win %' in df.columns:
                s['Win %'] = df['Win %'].apply(c_winpct)
            # TOTAL row → navy
            if 'TOTAL' in df.index:
                s.loc['TOTAL'] = 'background:#E8EDF5;color:#0F172A;font-weight:800'
            return s

        st.dataframe(
            df1.style.set_properties(**BASE).apply(sty_t1,axis=None)
               .format(lambda x: '' if isinstance(x,(int,float)) and x==0 else
                       (f'{x:,}' if isinstance(x,(int,float)) else x)),
            use_container_width=True, height=520)
        st.download_button("⬇ Download CSV", df1.to_csv(), "rep_source.csv","text/csv")

# ══════════════════════════════════════════════════════════════════════════════
# TAB 2 — SOURCE PERFORMANCE
# ══════════════════════════════════════════════════════════════════════════════
with t2:
    st.markdown("<div class='sec-hdr'>Source Performance — Conversion, Loss & Pipeline Quality</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Sorted by Win % descending. Shows which sources give quality leads "
                "vs which are generating volume with no conversion.</p>", unsafe_allow_html=True)

    src_rows = []
    for src in MAIN_SOURCES + ['Other']:
        sd = base[base['Source_group']==src]
        if len(sd)==0: continue
        n=len(sd); w=int((sd['Stage']=='Won').sum())
        l=int((sd['Stage']=='Lost').sum()); a=int((sd['Stage']=='Active').sum())
        qs=int(sd['FollowupStatus'].isin(HIGH_S).sum())
        rnr_n=int(sd['FollowupStatus'].isin(RNR_S).sum())
        src_rows.append({
            'Source':src, 'Leads':n,
            'Lead %':f'{round(n/total*100,1)}%',
            'Won':w, 'Lost':l, 'Active':a,
            'Quotes Sent':qs,
            'Win %':f'{round(w/n*100,1)}%',
            'Loss %':f'{round(l/n*100,1)}%',
            'Cold RNR':rnr_n,
            '_wr':round(w/n*100,1) if n else 0,
            '_lr':round(l/n*100,1) if n else 0,
        })
    src_rows.append({
        'Source':'TOTAL','Leads':total,
        'Lead %':'100%','Won':int(won),'Lost':int(lost),'Active':int(active),
        'Quotes Sent':int(quotes),
        'Win %':f'{wr}%','Loss %':f'{round(lost/total*100,1)}%',
        'Cold RNR':int(rnr),'_wr':wr,'_lr':round(lost/total*100,1),
    })
    df2 = pd.DataFrame(src_rows)
    df2 = pd.concat([df2[df2['Source']!='TOTAL'].sort_values('_wr',ascending=False),
                     df2[df2['Source']=='TOTAL']]).set_index('Source')

    def sty_t2(df):
        s=pd.DataFrame('',index=df.index,columns=df.columns)
        for i,row in df.iterrows():
            if i=='TOTAL':
                s.loc[i]='background:#E8EDF5;color:#0F172A;font-weight:800'; continue
            s.loc[i,'Win %']  = c_winpct(row['Win %'])
            s.loc[i,'Loss %'] = c_losspct(row['Loss %'])
            if row['Won']>0:
                s.loc[i,'Won']='background:#E9F7EF;color:#0A6640;font-weight:700'
            if i in ('Existing Client','Ex-Client Ref.'):
                s.loc[i,'Source']='background:#E9F7EF;color:#0A6640;font-weight:700'
        return s

    st.dataframe(
        df2.style.set_properties(**BASE)
           .apply(sty_t2,axis=None)
           .format({'Leads':'{:,}','Won':'{:,}','Lost':'{:,}',
                    'Active':'{:,}','Quotes Sent':'{:,}','Cold RNR':'{:,}'})
           .hide(axis=1, subset=['_wr','_lr']),
        use_container_width=True, height=420)
    st.download_button("⬇ Download CSV",df2.drop(columns=['_wr','_lr']).to_csv(),
                       "source_performance.csv","text/csv")

# ══════════════════════════════════════════════════════════════════════════════
# TAB 3 — ACTIVE LEAD AGING  (rep-wise buckets)
# ══════════════════════════════════════════════════════════════════════════════
with t3:
    st.markdown("<div class='sec-hdr'>Active Lead Aging — How Long Leads Have Been Sitting</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Active leads only, bucketed by age since lead creation date. "
                "🟢 Fresh (0–7 days)  🟡 Needs attention (8–15)  "
                "🟠 Getting stale (16–30)  🔴 Problem leads (30+ days)</p>",
                unsafe_allow_html=True)

    active_b = base[base['Stage']=='Active'].copy()
    if len(active_b)==0:
        st.info("No active leads in current filters.")
    else:
        if 'age_days' not in active_b.columns:
            st.warning("CreatedOn date not available in this export — cannot calculate aging.")
        else:
            active_b['bucket'] = active_b['age_days'].apply(age_bucket)
            BUCKETS = ['0–7 days','8–15 days','16–30 days','30+ days']
            aging = pd.crosstab(active_b['Rep'], active_b['bucket'], margins=True, margins_name='TOTAL')
            # Ensure all buckets present and ordered
            for b in BUCKETS:
                if b not in aging.columns: aging[b]=0
            col_ord = BUCKETS+[c for c in aging.columns if c not in BUCKETS and c!='TOTAL']
            if 'TOTAL' in aging.columns: col_ord.append('TOTAL')
            aging = aging[[c for c in col_ord if c in aging.columns]]

            def sty_t3(df):
                s=pd.DataFrame('',index=df.index,columns=df.columns)
                for col in df.columns:
                    if col=='TOTAL':
                        s[col]='background:#EAF0FB;color:#0F2044;font-weight:700'
                    elif col=='0–7 days':
                        s[col]=df[col].apply(lambda v:
                            'background:#E9F7EF;color:#0A6640;font-weight:600' if isinstance(v,(int,float)) and v>0 else '')
                    elif col=='8–15 days':
                        s[col]=df[col].apply(lambda v:
                            'background:#FFFBEB;color:#92400E;font-weight:600' if isinstance(v,(int,float)) and v>0 else '')
                    elif col=='16–30 days':
                        s[col]=df[col].apply(lambda v:
                            'background:#FEF3C7;color:#B45309;font-weight:700' if isinstance(v,(int,float)) and v>0 else '')
                    elif col=='30+ days':
                        s[col]=df[col].apply(lambda v:
                            'background:#FEF2F2;color:#B91C1C;font-weight:700' if isinstance(v,(int,float)) and v>0 else '')
                if 'TOTAL' in df.index:
                    s.loc['TOTAL']='background:#E8EDF5;color:#0F172A;font-weight:800'
                return s

            st.dataframe(
                aging.style.set_properties(**BASE).apply(sty_t3,axis=None)
                     .format(lambda x: '' if isinstance(x,(int,float)) and x==0 else f'{x:,}'),
                use_container_width=True, height=480)
            st.download_button("⬇ Download CSV",aging.to_csv(),"aging.csv","text/csv")

            # Summary: % of each rep's active leads that are 30+ days old
            st.markdown("<div class='sec-hdr'>Stale Lead % — Reps with Highest 30+ Day Active Leads</div>",
                        unsafe_allow_html=True)
            summ_rows=[]
            for rep in rep_order:
                rd=active_b[active_b['Rep']==rep]
                if len(rd)==0: continue
                n=len(rd)
                old=int((rd['bucket']=='30+ days').sum())
                summ_rows.append({
                    'Rep':rep,'Total Active':n,
                    '30+ Days':old,
                    'Stale %':f'{round(old/n*100,1)}%',
                    '_pct':round(old/n*100,1)
                })
            summ=pd.DataFrame(summ_rows).sort_values('_pct',ascending=False)

            def sty_summ(df):
                s=pd.DataFrame('',index=df.index,columns=df.columns)
                for i,row in df.iterrows():
                    p=row['_pct'] if '_pct' in row else 0
                    if p>=60: s.loc[i,'Stale %']='background:#FEF2F2;color:#B91C1C;font-weight:700'
                    elif p>=30: s.loc[i,'Stale %']='background:#FFFBEB;color:#92400E;font-weight:600'
                    else: s.loc[i,'Stale %']='background:#E9F7EF;color:#0A6640;font-weight:600'
                return s

            st.dataframe(
                summ[['Rep','Total Active','30+ Days','Stale %']]
                    .style.set_properties(**BASE).apply(sty_summ,axis=None)
                    .format({'Total Active':'{:,}','30+ Days':'{:,}'}),
                use_container_width=True, height=420)

# ══════════════════════════════════════════════════════════════════════════════
# TAB 4 — LOST REASONS BY REP
# ══════════════════════════════════════════════════════════════════════════════
with t4:
    st.markdown("<div class='sec-hdr'>Lost Reasons — Rep-wise Breakdown</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Cross-tab: rows = reps, columns = lost reasons. "
                "Darker red = higher count for that reason. TOTAL column = rep's total lost leads.</p>",
                unsafe_allow_html=True)

    lost_b=base[base['Stage']=='Lost']
    if len(lost_b)==0:
        st.info("No lost leads.")
    else:
        lc=pd.crosstab(lost_b['Rep'],lost_b['FollowupStatus'],margins=True,margins_name='TOTAL')
        if 'TOTAL' in lc.columns:
            lc=lc[[c for c in lc.columns if c!='TOTAL']+['TOTAL']]

        def sty_t4(df):
            s=pd.DataFrame('',index=df.index,columns=df.columns)
            for col in df.columns:
                if col=='TOTAL':
                    s[col]='background:#EAF0FB;color:#0F2044;font-weight:700'
                else:
                    mx=df[col].replace(0,np.nan).max()
                    if pd.notna(mx) and mx>0:
                        s[col]=df[col].apply(lambda v:(
                            'background:#FEF2F2;color:#B91C1C;font-weight:700' if v>=mx*0.6
                            else ('background:#FEF2F2;color:#B91C1C' if v>=mx*0.3
                                  else ('color:#B91C1C' if v>0 else ''))))
            if 'TOTAL' in df.index:
                s.loc['TOTAL']='background:#E8EDF5;color:#0F172A;font-weight:800'
            return s

        st.dataframe(
            lc.style.set_properties(**BASE).apply(sty_t4,axis=None)
              .format(lambda x: '' if isinstance(x,(int,float)) and x==0 else f'{x:,}'),
            use_container_width=True, height=480)
        st.download_button("⬇ Download CSV",lc.to_csv(),"lost_reasons.csv","text/csv")

        # Loss rate summary
        st.markdown("<div class='sec-hdr'>Loss Rate & Top Lost Reason per Rep</div>",
                    unsafe_allow_html=True)
        lr_rows=[]
        for rep in rep_order:
            rd=base[base['Rep']==rep]
            ld=rd[rd['Stage']=='Lost']
            n=len(rd); nl=len(ld)
            top=ld['FollowupStatus'].value_counts().index[0] if nl>0 else '—'
            lr_rows.append({'Rep':rep,'Total Leads':n,'Lost':nl,
                'Loss %':f'{round(nl/n*100,1)}%' if n else '0%',
                'Top Reason':top,'_lp':round(nl/n*100,1) if n else 0})
        lr_df=pd.DataFrame(lr_rows).sort_values('_lp',ascending=False)

        def sty_lr(df):
            s=pd.DataFrame('',index=df.index,columns=df.columns)
            for i,row in df.iterrows():
                p=row['_lp']
                if p>=70: s.loc[i,'Loss %']='background:#FEF2F2;color:#B91C1C;font-weight:700'
                elif p>=50: s.loc[i,'Loss %']='background:#FFFBEB;color:#92400E;font-weight:600'
                else: s.loc[i,'Loss %']='background:#E9F7EF;color:#0A6640;font-weight:600'
            return s

        st.dataframe(
            lr_df[['Rep','Total Leads','Lost','Loss %','Top Reason']]
                 .style.set_properties(**BASE).apply(sty_lr,axis=None)
                 .format({'Total Leads':'{:,}','Lost':'{:,}'}),
            use_container_width=True, height=420)

# ══════════════════════════════════════════════════════════════════════════════
# TAB 5 — STAGE FUNNEL BY REP
# ══════════════════════════════════════════════════════════════════════════════
with t5:
    st.markdown("<div class='sec-hdr'>Stage Funnel — Where Each Rep's Active Leads Are Right Now</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Active leads only. 🟢 Quoted stages (high intent)  "
                "🟡 Warm — Interested / Catalogue  🔴 Cold — Call Back / RNR</p>",
                unsafe_allow_html=True)

    act_b=base[base['Stage']=='Active']
    if len(act_b)==0:
        st.info("No active leads.")
    else:
        fc=pd.crosstab(act_b['Rep'],act_b['FollowupStatus'],margins=True,margins_name='TOTAL')
        cold_cols=[c for c in fc.columns if 'Call Back' in c or 'RNR' in c]
        hot_cols =[c for c in fc.columns if any(x in c for x in ['Quoted','Quote']) and c!='TOTAL']
        warm_cols=[c for c in fc.columns if c not in cold_cols+hot_cols+['TOTAL']]
        col_ord  =cold_cols+warm_cols+hot_cols+[c for c in fc.columns
                   if c not in cold_cols+warm_cols+hot_cols+['TOTAL']]
        if 'TOTAL' in fc.columns: col_ord.append('TOTAL')
        fc=fc[[c for c in col_ord if c in fc.columns]]

        def sty_t5(df):
            s=pd.DataFrame('',index=df.index,columns=df.columns)
            for col in df.columns:
                if col=='TOTAL':
                    s[col]='background:#EAF0FB;color:#0F2044;font-weight:700'
                elif col in cold_cols:
                    s[col]=df[col].apply(lambda v:
                        'background:#FEF2F2;color:#B91C1C;font-weight:700'
                        if isinstance(v,(int,float)) and v>0 else '')
                elif col in hot_cols:
                    s[col]=df[col].apply(lambda v:
                        'background:#E9F7EF;color:#0A6640;font-weight:700'
                        if isinstance(v,(int,float)) and v>0 else '')
                elif col in warm_cols:
                    s[col]=df[col].apply(lambda v:
                        'background:#FFFBEB;color:#92400E'
                        if isinstance(v,(int,float)) and v>0 else '')
            if 'TOTAL' in df.index:
                s.loc['TOTAL']='background:#E8EDF5;color:#0F172A;font-weight:800'
            return s

        st.dataframe(
            fc.style.set_properties(**BASE).apply(sty_t5,axis=None)
              .format(lambda x: '' if isinstance(x,(int,float)) and x==0 else f'{x:,}'),
            use_container_width=True, height=480)
        st.download_button("⬇ Download CSV",fc.to_csv(),"stage_funnel.csv","text/csv")

        # Funnel summary per rep
        st.markdown("<div class='sec-hdr'>Funnel Quality Summary per Rep</div>",
                    unsafe_allow_html=True)
        fs_rows=[]
        for rep in rep_order:
            rd=act_b[act_b['Rep']==rep]; n=len(rd)
            if n==0: continue
            cold_n=int(rd['FollowupStatus'].isin(cold_cols).sum())
            hot_n =int(rd['FollowupStatus'].isin(hot_cols).sum())
            warm_n=n-cold_n-hot_n
            fs_rows.append({'Rep':rep,'Active':n,
                'Cold CB/RNR':f'{cold_n} ({round(cold_n/n*100,1)}%)',
                'High Intent':f'{hot_n}  ({round(hot_n/n*100,1)}%)',
                'Warm':f'{warm_n}  ({round(warm_n/n*100,1)}%)',
                '_hot':round(hot_n/n*100,1),'_cold':round(cold_n/n*100,1)})
        fs=pd.DataFrame(fs_rows).sort_values('_hot',ascending=False)

        def sty_fs(df):
            s=pd.DataFrame('',index=df.index,columns=df.columns)
            for i,row in df.iterrows():
                if row['_cold']>=60: s.loc[i,'Cold CB/RNR']='background:#FEF2F2;color:#B91C1C;font-weight:700'
                if row['_hot']>=15:  s.loc[i,'High Intent']='background:#E9F7EF;color:#0A6640;font-weight:700'
            return s

        st.dataframe(
            fs[['Rep','Active','Cold CB/RNR','High Intent','Warm']]
              .style.set_properties(**BASE).apply(sty_fs,axis=None)
              .format({'Active':'{:,}'}),
            use_container_width=True, height=420)

# ══════════════════════════════════════════════════════════════════════════════
# TAB 6 — RNR AGING BY REP
# ══════════════════════════════════════════════════════════════════════════════
with t6:
    st.markdown("<div class='sec-hdr'>RNR & Call Back Aging — How Long Stuck Leads Have Been Sitting</div>",
                unsafe_allow_html=True)
    st.markdown("<p class='sub-note'>"
                "Leads currently in Call Back or RNR Call Back status, bucketed by age. "
                "🟡 0–3 days (normal)  🟠 4–7 days (needs call)  "
                "🔴 8–15 days (overdue)  🚨 15+ days (abandon or escalate)</p>",
                unsafe_allow_html=True)

    rnr_b=base[base['FollowupStatus'].isin(RNR_S)].copy()
    if len(rnr_b)==0:
        st.info("No RNR / Call Back leads in current filters.")
    else:
        if 'age_days' not in rnr_b.columns:
            st.warning("CreatedOn not available — cannot calculate RNR aging.")
        else:
            rnr_b['bucket']=rnr_b['age_days'].apply(rnr_bucket)
            RNR_BUCKETS=['0–3 days','4–7 days','8–15 days','15+ days']
            ra=pd.crosstab(rnr_b['Rep'],rnr_b['bucket'],margins=True,margins_name='TOTAL')
            for b in RNR_BUCKETS:
                if b not in ra.columns: ra[b]=0
            col_ord2=RNR_BUCKETS+[c for c in ra.columns if c not in RNR_BUCKETS and c!='TOTAL']
            if 'TOTAL' in ra.columns: col_ord2.append('TOTAL')
            ra=ra[[c for c in col_ord2 if c in ra.columns]]

            def sty_t6(df):
                s=pd.DataFrame('',index=df.index,columns=df.columns)
                clr={'0–3 days':('background:#FFFBEB','color:#92400E','font-weight:600'),
                     '4–7 days':('background:#FEF3C7','color:#B45309','font-weight:700'),
                     '8–15 days':('background:#FEF2F2','color:#B91C1C','font-weight:700'),
                     '15+ days':('background:#7F1D1D','color:white','font-weight:700')}
                for col in df.columns:
                    if col=='TOTAL':
                        s[col]='background:#EAF0FB;color:#0F2044;font-weight:700'
                    elif col in clr:
                        bg,fc_,fw=clr[col]
                        s[col]=df[col].apply(lambda v:
                            f'{bg};{fc_};{fw}' if isinstance(v,(int,float)) and v>0 else '')
                if 'TOTAL' in df.index:
                    s.loc['TOTAL']='background:#E8EDF5;color:#0F172A;font-weight:800'
                return s

            st.dataframe(
                ra.style.set_properties(**BASE).apply(sty_t6,axis=None)
                  .format(lambda x: '' if isinstance(x,(int,float)) and x==0 else f'{x:,}'),
                use_container_width=True, height=480)
            st.download_button("⬇ Download CSV",ra.to_csv(),"rnr_aging.csv","text/csv")

            # RNR summary per rep
            st.markdown("<div class='sec-hdr'>RNR Severity per Rep — Who Has the Most Stuck Leads</div>",
                        unsafe_allow_html=True)
            rr_rows=[]
            for rep in rep_order:
                rd=rnr_b[rnr_b['Rep']==rep]; n=len(rd)
                if n==0: continue
                old=int((rd['bucket']=='15+ days').sum())
                med=int((rd['bucket']=='8–15 days').sum())
                total_rep=len(base[base['Rep']==rep])
                rr_rows.append({
                    'Rep':rep,'Total Active':len(base[(base['Rep']==rep)&(base['Stage']=='Active')]),
                    'In RNR/CB':n,
                    'RNR %':f'{round(n/total_rep*100,1)}%' if total_rep else '0%',
                    '8–15 days':med,'15+ days':old,
                    '_rnr_pct':round(n/total_rep*100,1) if total_rep else 0,
                    '_old':old,
                })
            rr=pd.DataFrame(rr_rows).sort_values('_old',ascending=False)

            def sty_rr(df):
                s=pd.DataFrame('',index=df.index,columns=df.columns)
                for i,row in df.iterrows():
                    if row['_old']>20: s.loc[i,'15+ days']='background:#7F1D1D;color:white;font-weight:700'
                    elif row['_old']>5: s.loc[i,'15+ days']='background:#FEF2F2;color:#B91C1C;font-weight:700'
                    if row['_rnr_pct']>=50: s.loc[i,'RNR %']='background:#FEF2F2;color:#B91C1C;font-weight:700'
                    elif row['_rnr_pct']>=30: s.loc[i,'RNR %']='background:#FFFBEB;color:#92400E;font-weight:600'
                return s

            st.dataframe(
                rr[['Rep','Total Active','In RNR/CB','RNR %','8–15 days','15+ days']]
                  .style.set_properties(**BASE).apply(sty_rr,axis=None)
                  .format({'Total Active':'{:,}','In RNR/CB':'{:,}','8–15 days':'{:,}','15+ days':'{:,}'}),
                use_container_width=True, height=420)

# ── FOOTER ────────────────────────────────────────────────────────────────────
st.markdown("---")
st.markdown(
    f"<p style='text-align:center;color:#94A3B8;font-size:11px;'>"
    f"MSafe Equipments  |  KIT19 CRM  |  {len(df_raw):,} leads in file  |  "
    f"{len(base):,} leads shown with current filters</p>", unsafe_allow_html=True)
